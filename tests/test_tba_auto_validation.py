...
class _DummyAsyncClientParkNone(_DummyAsyncClient):
    async def get(self, url, headers=None):
        return _DummyResponse(
            {
                "score_breakdown": {
...
@pytest.mark.asyncio
async def test_alliance_validations_allow_park_none_discrepancy(monkeypatch):
    monkeypatch.setenv("TBA_API_KEY", "test-key")
    monkeypatch.setattr(
        "app.services.scout.httpx.AsyncClient", _DummyAsyncClientParkNone
    )
